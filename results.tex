...
\Cref{fig:avg-local-cc-full} depicts the development of \( C(t) \) over the course of the simulation for different levels of peer influence.
...
Nevertheless, weak ties are eventually introduced to the network by the focal closure mechanism as well.
...
\Cref{fig:percentage-reinforced-ties} shows the plots for the percentage of reinforcement activity over time in the initial phase of the simulation and over all 75,000 iterations.
...
The evolution of the average local clustering coefficient does depend on the node deletion probability \( p_{d} \), since low-activity nodes, which are not removed fast enough, introduce additional weak ties in the network~\cite{Laurent2015}.
...
However, the time it takes for them to converge differs.
...
\Cref{fig:inter-event-time-dist-start} depicts the distributions in this interval of \( \tau \).
...
\Cref{fig:activity-beta-scenarios} shows the number of interactions between nodes and the fraction of peer influenced interactions as functions of time for the three different scenarios.
