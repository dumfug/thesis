...
Each weight in the egocentric networks of \( v_{i} \) is transformed and normalized by \( w'_{i,j} = \sfrac{\exp(\beta w_{i,j})}{\sum_{k \in N(i)} \exp(\beta w_{i,k})} \), where \( \beta \) is a free parameter.
...
The free parameter \( \beta \) in the softmax function called is inverse temperature and replicates this behavior.
...
For instance, it is used in the machine learning area of reinforcement learning, where the actions of agents in a setting are based on the probabilities yielded by the softmax function.
