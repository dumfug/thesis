...
The additional introduced mechanisms, that allow for peer influenced activity to actually happen in the network, are then discussed in \cref{sec:peer-influence-model} of this chapter.
...
Let \( k_{i} = d(v_{i}) \) be the degree of the node \( v_{i} \) in its egocentric network, then the probability to form a new tie is \( p(k_{i}) = \frac{c}{k_{i} + c} \), and the probability to interact with an already established tie is \( \bar{p}(k_{i}) = 1 - p(k_{i}) = 1 - \frac{c}{k_{i} + c} = \frac{k_{i}}{k_{i} + c} \), where the constant \(c\) determines how strong the memory of an actor is (cf.  \cref{fig:reinforcement-process-prob-plot} for details).
...
\todo{move these two sentences into the results chapter}
