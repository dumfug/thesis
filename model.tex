...
\begin{figure}
    ...
    \caption[Extended egocentric network example]{Extented egocentric network of the node \(v_{i} \) (red node) and its neighbors \( v_{j} \), \( v_{k} \), and \( v_{l} \) (blue nodes).}
    \label{fig:egocentric-network}
\end{figure}
...
\( \delta \) & The constnat factor that is added to the tie strength when it is reinforced. \\
