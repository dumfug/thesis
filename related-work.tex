...
\todo{naming games. (shaping of opinions)}
Another work by \citet{Estrada2013} studies the effect of peer influence in the shaping of opinions in social groups.
...
This allows for a node to receive indirect peer pressure from other nodes that are not directly connected to it (i.e., do not share a edge).
...
While peer influence is a prominent sociology topic, it is not heavily common to consider peer influence effects in network models.
...
In their work they perform a linear stability analysis on the system of differential equations and show that stability of the system is determined by the relation between the activity dynamics ratio \( \frac{\lambda}{\mu} \) (with \( \mu = \frac{q}{a_{c}} \)) and the topological structure of the network.
...
The second measure is called activity momentum and is defined as product of the system mass and the over some time span averaged last observed activity in the network., and is an improved indicator of the robustness.
For instance, a system with a large activity momentum value handles the sudden inactivity of a considerable number of users better by keeping most of its activity.
Therefore, much more \enquote{force} is needed to render a network with high activity momentum inactive.

Another interesting question was raised in the work of \citet{Aral2009}.
...
Homophily (also known as assortativity or assortative mixing) is the effect that user tend to interact with other user that are similar to them self.
...
For instance, users that have the property of being early-adopters are more likely to be early-adopters as well, and therefore, start using the feature at about the same time.
They proposed a statistical framework in their work, that is able to distinguish between peer influence and homophily, and showed that other methods overestimate the share of influenced adaption by a factor of four to eight, and that at least half of the adaption in their instant-messaging data set is introduced by homophily.
